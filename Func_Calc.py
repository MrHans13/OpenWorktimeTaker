...
def calc_Work_Time():
    work_minutes = daF.get_int_Data('temp/.time_file.txt', 'stopmin') - daF.get_int_Data('temp/.time_file.txt', 'startmin')
    work_hours = daF.get_int_Data('temp/.time_file.txt', 'stophour') - daF.get_int_Data('temp/.time_file.txt', 'starthour')
    work_minutes_ges = daF.get_int_Data('temp/.time_file.txt', 'workminges')
    daF.set_data('temp/.time_file.txt', 'workminges', str(work_minutes_ges + work_minutes))
    work_hours_ges = daF.get_int_Data('temp/.time_file.txt', 'workhourges')
    daF.set_data('temp/.time_file.txt', 'workhourges', str(work_hours_ges + work_hours))
    ...
    if work_minutes < 0:
        work_minutes += 60
        work_hours -= 1
    daF.set_data('temp/.time_file.txt', 'workmin', work_minutes)
    daF.set_data('temp/.time_file.txt', 'workhour', work_hours)
    calc_O_Time()


def calc_O_Time():
    daystate = daF.get_int_Data('temp/.prog_states.txt', 'day_state')
